...
import { ArrowLeft, Send, Search, X, Mail, MessageSquare, Lock, Receipt, AlertTriangle, Paperclip } from 'lucide-react';
...
export const InvoiceReadyPage: React.FC = () => {
...
Your vehicle service invoice is now ready.

Invoice No: ${selectedInvoice.invoiceNumber}
Total: ${formatLKR(selectedInvoice.grandTotal)}
Outstanding: ${formatLKR(selectedInvoice.outstandingBalance)}
...
Your invoice is attached to this email.`;
  ...
  const validateNotification = () => {
    ...
    if (selectedInvoice.status !== 'sent' && selectedInvoice.status !== 'paid') {
      setValidationError('Invoice must be approved before sending an invoice notification');
      return false;
    }
  ...
  const handleSendNotification = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success('Invoice ready notification sent successfully');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <h1 className="text-2xl font-bold text-slate-900">🧾 Invoice Ready</h1>
        <div className="w-32" />
      </div>
          ...
          <>
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-2 p-3 border border-slate-200 rounded-xl bg-slate-50">
                ...
                <span className="text-sm text-slate-600">{selectedInvoice.invoiceNumber}.pdf</span>
              </div>
            </div>
          </>
        )}
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {invoiceResults.map((invoice) => (
                ...
                >
                  ...
                  <p className="text-sm text-slate-600">Total: {formatLKR(invoice.grandTotal)}</p>
                  <p className="text-sm text-slate-600">Status: {invoice.status}</p>
                </div>
              ))}
              {invoiceResults.length === 0 && invoiceSearchQuery && (
                <p className="text-center text-sm text-slate-400 py-4">No invoices found</p>
              )}
            </div>
          </div>
        </div>

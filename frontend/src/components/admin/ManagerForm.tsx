...
import { formatInputValue, parseNumberInput } from '../../utils/formatters';
...
export const ManagerForm: React.FC<ManagerFormProps> = ({
  ...
  useEffect(() => {
    const fetchNextUserId = async () => {
      try {
        const res = await userApi.getNextUserId('manager');
        if (res.success) {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        role: 'manager',
        nic: initialData.profile?.nic || initialData.nic || '',
        dateOfBirth: initialData.dateOfBirth || '1995-01-01',
        gender: initialData.gender || 'male',
        designation: initialData.profile?.designation || initialData.designation || 'Manager',
        basicSalary: initialData.profile?.basicSalary || initialData.basicSalary || 120000,
        employmentDate: initialData.profile?.employmentDate || initialData.employmentDate || new Date().toISOString().split('T')[0],
        address: initialData.address || initialData.profile?.address || { street: '', city: '', province: '', postalCode: '' },
        bankName: initialData.profile?.bankName || '',
        branch: initialData.profile?.branch || '',
        accountNumber: initialData.profile?.accountNumber || '',
        confirmAccountNumber: initialData.profile?.accountNumber || '',
        profilePhoto: null,
        isActive: initialData.isActive !== undefined ? initialData.isActive : true,
      });
      
      if (initialData.managerId) {
        setNextUserId(initialData.managerId);
      }
    }
  }, [initialData]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 text-xs font-medium">
      ...
      <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
        ...
        <div className="text-[10px] text-slate-400 mt-1">System Generated</div>
      </div>
      ...
      <div>
        ...
        <input
          ...
          className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500/20 mt-2"
        />
      </div>
      ...
      <div>
        ...
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-xs font-medium">LKR</span>
          <input
            type="number"
            required
            value={formatInputValue(formData.basicSalary)}
            onChange={(e) => setFormData({ ...formData, basicSalary: parseNumberInput(e.target.value) })}
            placeholder="120000"
            className="w-full pl-10 p-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500/20"
          />
        </div>
      </div>
      ...
      {/* Profile Photo */}
      <div>
        <label className="block text-slate-700 mb-1 font-bold">Profile Photo</label>
        <input
          type="file"
          accept="image/*"
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-slate-700 mb-1 font-bold">Account No</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-slate-700 mb-1 font-bold">Confirm Account No</label>
            <input
              type="text"
